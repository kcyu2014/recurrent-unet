...
class unet_no_final_layer(nn.Module):
    def __init__(
        self,
        feature_scale=4,
        out_channels=4,
        ...
        super(unet_no_final_layer, self).__init__()
        self.is_deconv = is_deconv
        self.in_channels = in_channels
        self.is_batchnorm = is_batchnorm
        self.feature_scale = feature_scale
...
class ConvGRU(nn.Module):
    ...
    def __init__(self, args, input_size, hidden_size, n_classes):
        ...
        # self.reset_gate = unet_no_final_layer(in_channels=input_size + n_classes, out_channels=hidden_size)
        # self.update_gate = unet_no_final_layer(in_channels=input_size + n_classes, out_channels=hidden_size)
        # self.out_gate = unet_no_final_layer(in_channels=input_size + n_classes, out_channels=hidden_size)
        self.update_gate = unet_gn(in_channels=input_size + n_classes, out_channels=hidden_size)
        self.out_gate = unet_gn(in_channels=input_size + n_classes, out_channels=hidden_size)
    ...
    def forward(self, input_, prev_state, prev_h):
        ...
        stacked_inputs_ = torch.cat([input_, prev_h], dim=1)
...
class ConvGRU_parameterized_sum(nn.Module):
    """
    Generate a convolutional GRU cell
    """

    def __init__(self, args, input_size, hidden_size, n_classes):
        super(ConvGRU_parameterized_sum, self).__init__()
        self.device = args.device
        self.initial = args.initial
        self.input_size = input_size
        self.hidden_size = hidden_size
        ...
        self.update_gate = unet_gn(in_channels=input_size + n_classes, out_channels=hidden_size)
        self.out_gate = unet_gn(in_channels=input_size + n_classes, out_channels=hidden_size)
        self.convsum = MergeParametric(in_channels=hidden_size * 2, out_channels=hidden_size, kernel_size=1)
        self.gn = nn.GroupNorm(min(8, hidden_size), hidden_size)
        self.conv_down = nn.Conv2d(hidden_size, n_classes, 1)
    ...
    def forward(self, input_, prev_state, prev_h):
        ...
        # stacked_inputs_h = torch.cat([input_up, prev_state * reset], dim=1)
        out_inputs = torch.tanh(self.out_gate(stacked_inputs_))
        new_state = self.convsum(torch.cat((prev_state * (1 - update), out_inputs * update), 1))
        new_h = self.conv_down(self.gn(new_state))
        return new_state, new_h


class ConvGRU_gn(nn.Module):
    """
    Generate a convolutional GRU cell
    """

    def __init__(self, args, input_size, hidden_size, n_classes):
        super(ConvGRU_gn, self).__init__()
        self.device = args.device
        self.initial = args.initial
        self.input_size = input_size
        self.hidden_size = hidden_size
        ...
        # self.update_gate = unet_no_final_layer(in_channels=input_size + n_classes, out_channels=hidden_size)
        # self.out_gate = unet_no_final_layer(in_channels=input_size + n_classes, out_channels=hidden_size)
        self.update_gate = unet(in_channels=input_size + n_classes, out_channels=hidden_size)
        self.out_gate = unet(in_channels=input_size + n_classes, out_channels=hidden_size)
        self.gn = nn.GroupNorm(min(8, hidden_size), hidden_size)
        self.conv_down = nn.Conv2d(hidden_size, n_classes, 1)
        # self.conv_up = nn.Conv2d(input_size, hidden_size, 1)
    ...
    def forward(self, input_, prev_state, prev_h):
        ...
        out_inputs = torch.tanh(self.out_gate(stacked_inputs_))
        new_state = prev_state * (1 - update) + out_inputs * update
        new_state_gn = self.gn(new_state)
        new_h = self.conv_down(new_state_gn)
