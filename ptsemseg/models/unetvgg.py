...
class ConvReluGN(nn.Module):
    def __init__(self, in_: int, out: int):
        super(ConvReluGN, self).__init__()
        self.conv = conv3x3(in_, out)
        self.normlayer = nn.GroupNorm(8, out)
        self.activation = nn.ReLU(inplace=True)
...
class DecoderBlockGN(nn.Module):
    """
    Paramaters for Deconvolution were chosen to avoid artifacts, following
    link https://distill.pub/2016/deconv-checkerboard/
    """
    ...
    def __init__(self, in_channels, middle_channels, out_channels, is_deconv=True):
        super(DecoderBlockGN, self).__init__()
        self.in_channels = in_channels

        if is_deconv:
            self.block = nn.Sequential(
                ConvReluGN(in_channels, middle_channels),
                nn.ConvTranspose2d(middle_channels, out_channels, kernel_size=4, stride=2,
                                   padding=1),
            ...
            self.block = nn.Sequential(
                nn.Upsample(scale_factor=2, mode='bilinear'),
                ConvReluGN(in_channels, middle_channels),
                ConvReluGN(middle_channels, out_channels),
            )
...
class UNet11(nn.Module):
    def __init__(self, n_classes=1, num_filters=32, pretrained=True):
        """
        :param n_classes:
        :param num_filters:
        :param pretrained:
...
class UNet16(nn.Module):
    def __init__(self, n_classes=1, num_filters=32, pretrained=True):
        ...
        self.paramGroup2 = nn.Sequential(self.center, self.dec5, self.dec4, self.dec3, self.dec2, self.dec1, self.final)
...
class UNet16GN(nn.Module):
    def __init__(self, n_classes=1, num_filters=32, pretrained=True):
        ...
        self.center = DecoderBlockGN(512, num_filters * 8 * 2, num_filters * 8)

        self.dec5 = DecoderBlockGN(512 + num_filters * 8, num_filters * 8 * 2, num_filters * 8)
        self.dec4 = DecoderBlockGN(512 + num_filters * 8, num_filters * 8 * 2, num_filters * 8)
        self.dec3 = DecoderBlockGN(256 + num_filters * 8, num_filters * 4 * 2, num_filters * 2)
        self.dec2 = DecoderBlockGN(128 + num_filters * 2, num_filters * 2 * 2, num_filters)
        self.dec1 = ConvReluGN(64 + num_filters, num_filters)
        self.final = nn.Conv2d(num_filters, n_classes, kernel_size=1)
...
class UNet(nn.Module):
    ...
    def __init__(self,
        ...
        if add_output:
            self.conv_final = nn.Conv2d(up_filter_sizes[0], n_classes, 1)
...
class AlbuNet(nn.Module):
    ...
    def __init__(self, n_classes=1, num_filters=32, pretrained=False, is_deconv=False):
        """
        :param n_classes:
        :param num_filters:
        :param pretrained:
