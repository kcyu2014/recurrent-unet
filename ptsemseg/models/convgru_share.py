...
class ConvGRU(nn.Module):
    """
    ConvGRU with all 3 gates, with Group Normalization.
    ...
    def __init__(self, args, input_size, hidden_size, n_classes):
        ...
        self.reset_gate = unet_gn(in_channels=input_size + n_classes, out_channels=n_classes)
    ...
    def forward(self, input_, prev_state, prev_h):
        ...
        # Use this as reset gates.
        reset = torch.sigmoid(self.reset_gate(stacked_inputs_))

        reset_h = reset * prev_h
        stacked_reset_inputs_ = torch.cat([input_, reset_h], dim=1)
        ...
        out_inputs = torch.tanh(self.out_gate(stacked_reset_inputs_))
